...
const authController = {
  async register(req, res, next) {
    ...
    // 3. if email and username is already resgister -> return an error

    const { username, name, email, password } = req.body;

    try {
      const emialInUse = await User.exists({ email });
      ...
      if (usernameInUse) {
        const error = {
          status: 409,
          message: "Username is already exists, please use another username!",
        };
        return next(error);
      }
    } catch (error) {
